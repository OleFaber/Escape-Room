#if uebergang == True:
import time # Importieren des Zeitmoduls, um Pausen zwischen den print-Befehlen zu erzeugen
...
def gefaengnishof_1():
    ...
    time.sleep(2)
    print("Der Wachturm ist verschlossen und man benötigt einen Schlüssel, um hineinzukommen.") # Einleitung in das Thema des Raumes
    ...
    while True:
        ...
        kommando = input("Zu welcher Station möchtest du gehen? (Treppe/Kiste/Tisch/Wachturm): ").lower() # Spieler wählt Station
        time.sleep(2)

        # Überprüfen der Eingabe (Station)
        if kommando == "treppe":
            if not schluessel_gefunden: # Überprüfen, ob Schlüssel bereits gefunden
                print("Glückwunsch! Unter der Treppe liegt ein Schlüssel,den ein Wachmann dort verloren haben muss.")
                time.sleep(2)
            ...
            if not tuer_wachturm: # Überprüfen, ob Wachturm bereits betreten wurde
                ...
                if schluessel_gefunden and teilcode_gefunden and teilcode2_gefunden:
                    print("Du hast alle Hinweise in den Stationen sowie den Schlüssel gefunden und kommst somit in die nächste Stufe.")
...
def gefaengnishof_2_wachturm():
    ...
    while not seil_gefunden: # Schleife, die läuft, bis das Seil gefunden wurde.
        ...
        if kommando == "schreibtisch": # Spieler untersucht den Schreibtisch.
            ...
            if not werkzeugkoffer_durchsucht: # Werkzeugkoffer wurde noch nicht durchsucht
                print("Im Werkzeugkoffer sind ein paar Maulschlüssel, sowie ein paar kleinere Bügelschellen, ansonsten ist er leer")
            ...
            if not seil_gefunden:
                if code_gefunden == True and werkzeugkoffer_durchsucht == True: # Überprüft, ob alle Hinweise gefunden wurden
                    ...
                    time.sleep(2)
                    print("Vielleicht bringt dich der Code, den du gerade notiert hast, weiter", code.get("CODE_Teil3"))
    ...
    while True: # Der Spieler trifft eine Entscheidung, um aus dem Gefängnis zu fliehen
        vorgehen = input("Möchtest du die Bügelschellen holen, das Seil anknoten oder dir eine Alternative suchen? Knoten / Schelle / Alternative: ").lower()
        if vorgehen == "knoten":
            print("Du knotest das Seil an kletterst aus dem Fester und während des Abseilens löst sich das Seil vom Geländer und du fällst in die Tiefe und stirbst")
            time.sleep(2)
